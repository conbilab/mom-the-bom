# 몸 더 봄 홈페이지 디자인 시스템

## 1. Brief

- 브랜드: `몸 더 봄` (`MOM THE BOM`)
- 의미: 인간의 몸을 더 자세히 바라보고, 매일의 선택을 통해 웰빙과 웰니스를 돕는 브랜드.
- 홈페이지 역할: 국내 유통사와 해외 파트너가 첫 화면에서 브랜드의 수준과 확장성을 판단할 수 있는 이미지 중심 브랜드 사이트.
- 1차 전환: 파트너십 제안.
- 2차 전환: 브랜드 철학과 첫 제품 이해.
- 현재 제품: 몸더봄 산야초 건성 두피용 샴푸, 몸더봄 육미지황 지성 두피용 샴푸, 모앤봄 클린 앤 스테이 퍼퓸. 산야초 상세페이지는 확인 가능한 시험 근거를 보여주는 대표 제품 사례로 유지하고, 퍼퓸 상세페이지는 휴대성과 사용법, 제품 정보, 가격을 보여주는 두 번째 생활 기준으로 확장한다.

## 2. Research Log

### 볼트 브랜드 자료

- `00_모앤봄_실행인덱스.md`: 기능성 탈모샴푸 정면 경쟁보다 매일 쓰는 제품을 좋은 것으로 바꾸는 생활 케어에 집중한다.
- `2026-모앤봄-진정성스토리-포장-두피서비스-빌드업.md`: 20년 넘게 현장에서 민감한 고민을 들은 경험, 과장보다 매일 닿는 기준, 제품 이후에도 이어지는 돌봄이 브랜드의 원천이다.
- `2026-모앤봄-경쟁사분석-전략보고서.md`: 경쟁사의 강한 증거 배치 문법은 배우되, 몸 더 봄은 산야초, 생활 루틴, 조용한 증거, 선물의 언어로 차별화한다.
- `모앤봄_상세페이지_광고_표시광고_컴플라이언스_가이드.md`: 피부 자극지수 0.00은 인체피부 일차자극 시험의 결과로만 정확히 표현한다. 탈모, 발모, 질환, 치료 효과로 확장하지 않는다.

### 학습 자료

- 작은 브랜드 리서치: 박웅현의 `잘 말해진 진실`과 `Only One`, 이제석의 한 컷 직관, 노희영의 팔리는 한 끗을 결합한다.
- UI/UX 디자인 학습: 한 화면에 하나의 핵심 행동, 44px 이상 터치 영역, 4/8px 리듬, 명확한 포커스, 반응형 우선.
- UI UX DB 제안: 프리미엄 블랙과 골드 계열은 채택. Liquid Glass는 대비와 성능 위험 때문에 제외.

### 실제 화면 리서치

- Lazyweb `Grove Collaborative`: 제품을 큰 이미지로 먼저 보여주고, 제품군과 브랜드 근거가 바로 이어지는 구조를 참고한다.
- Lazyweb `Prose`: 제품의 재료감과 여백이 설명보다 먼저 브랜드를 말하는 방식을 참고한다.
- Apple reference: 화면 장식보다 사진과 소재가 주인공이 되는 dark/light chapter rhythm을 채택한다.
- Layer A: `soft-skill.md`의 Soft Structuralism. 과한 글래스, 오브, 카드 중첩은 제외한다.
- Layer B: `apple.md`. 제품 중심 이미지, 조용한 UI, 넓은 여백, 흑백 장면 전환을 적용한다.

## 3. Direction

### Concept Spine

`Living Standard`: 몸을 고쳐야 할 문제로 보지 않고, 더 잘 바라보며 매일의 기준을 세우는 살아 있는 시스템.

### Hero

- Giant image-first hero.
- 제품이 보이되 제품 상세페이지처럼 시작하지 않는다.
- 밝은 건축적 욕실과 아침 루틴을 통해 몸, 생활, 제품을 한 장면에 묶는다.
- 핵심 문장: `몸을 더 바라볼수록, 삶은 더 좋아집니다.`
- 첫 화면 아래에 다음 백색 섹션이 살짝 보여야 한다.

### Section Rhythm

1. Hero: 밝은 전체 화면 이미지, 왼쪽 아래 문장.
2. Manifesto: 백색 여백과 짧은 선언.
3. Four observations: 네 개의 생활 영역을 선형 목록으로 전개.
4. First proof: 어두운 물과 제품 이미지.
5. Real voices: 실제 후기 원문에서 사용감만 발췌한 고객 목소리.
6. Evidence: 조용한 수치와 정확한 주석.
7. Origin story: 사람의 배려가 보이는 아침 장면.
8. Partners: 짙은 소나무색 위에 파트너 가치.
9. Contact: 넓은 백색 문의 영역과 푸터.

제품 상세페이지에서는 제품 정보 뒤에 후기 공간 QR 진입점을 두고, 별도의 공개 후기 페이지로 연결한다.

## 4. Personas

- 국내 유통 담당자: 첫 10초 안에 브랜드의 차별성, 제품성, 협업 준비도를 판단한다.
- 해외 바이어: 한국적이되 전통 장식에 갇히지 않은 현대 웰니스 브랜드를 찾는다. 영어 전환과 제품 근거를 요구한다.
- 35~49세 생활 큐레이터: 가족에게 관리하라고 말하기보다 매일 쓰는 좋은 것을 건네고 싶다.

## 5. Tokens

### Color

| Token | Value | Role |
|---|---:|---|
| `--ink` | `#101712` | 주 텍스트, 어두운 장면 |
| `--pine` | `#123b2d` | 브랜드 앵커 |
| `--paper` | `#f4f4ef` | 주 배경 |
| `--porcelain` | `#ffffff` | 밝은 표면 |
| `--mist` | `#e7ebe7` | 보조 표면 |
| `--stone` | `#626b66` | 보조 텍스트 |
| `--amber` | `#b87923` | 제품 액체와 연결되는 제한적 강조 |
| `--blush` | `#c78187` | 클린 앤 스테이 퍼퓸의 제한적 제품 강조 |
| `--blush-mist` | `#f3e5e5` | 퍼퓸 페이지의 밝은 보조 표면 |
| `--line` | `#d5dad5` | 구분선 |
| `--focus` | `#d79b3f` | 키보드 포커스 |

### Typography

- Display: `KoPub Batang`, `Batang`, `AppleMyungjo`, `Georgia`, serif. 선언과 브랜드 철학에 사용하며 운영체제의 로컬 서체로 안정적으로 폴백한다.
- Text: `Pretendard`, `SUIT`, `Malgun Gothic`, `Arial`, sans-serif. 내비게이션, 본문, 폼, 데이터에 사용하며 외부 폰트 요청 없이 렌더링한다.
- `Ritual Steps`처럼 24px 이하의 실용 정보 제목은 Windows에서 구형 명조 폴백이 거칠게 보이지 않도록 Text 서체를 600 굵기로 사용한다. Display 서체는 큰 선언과 브랜드 철학에만 사용한다.
- 크기: 12 / 14 / 16 / 18 / 24 / 32 / 48 / 64 / 80px.
- 모든 letter-spacing은 `0`.
- 본문 line-height는 1.65, 긴 문장 너비는 70ch 이하.

### Spacing

- Base unit: 4px.
- Component: 8 / 12 / 16 / 24 / 32px.
- Section: 64 / 96 / 128 / 160px.
- Desktop container: 1280px, 좌우 최소 32px.
- Mobile gutter: 20px.

### Radius and Depth

- 버튼과 입력: 4px.
- 이미지 프레임: 0 또는 4px.
- 표면 분리는 색과 1px hairline으로 처리한다.
- box shadow는 내비게이션과 포커스된 오버레이 외에는 사용하지 않는다.

## 6. Components and States

### Brand Mark

- 한글 `몸 더 봄`과 영문 `MOM THE BOM`을 세로로 조합한다.
- 모든 화면에서 텍스트 로고로 선명하게 유지한다.

### Navigation

- 투명 오버레이에서 스크롤 후 잉크색 바탕으로 전환.
- 데스크톱: 브랜드, 4개 링크, 언어 전환, 파트너 CTA.
- 모바일: 메뉴 버튼이 오른쪽 drawer를 연다. Escape, 바깥 클릭, 링크 선택으로 닫힌다.

### CTA

- Primary: pine 또는 porcelain fill, 최소 높이 48px.
- Secondary: underline text link.
- States: default, hover, pressed scale 0.98, focus ring, disabled.
- beui.dev `button`의 press feedback을 CSS로 축약 적용한다.

### Observation Row

- 번호, 영역명, 한 문장, 세부 문장을 선형으로 배치한다.
- 클릭과 키보드로 열고 닫는다. hover만으로 정보를 숨기지 않는다.

### Evidence Block

- `0.00` 하나만 크게 보여준다. 가짜 KPI 배열을 만들지 않는다.
- 같은 화면 안에 시험 범위와 한계를 반드시 표시한다.

### Product Detail Hero

- `product-water.webp`를 전체 화면 배경으로 사용하고 제품명, 데일리 리추얼 문장, 검증된 핵심 사실을 첫 화면에 배치한다.
- 핵심 사실은 `250 ml`, `Made in Korea`, `피부 자극지수 0.00`만 사용한다.
- 구매 가격이나 재고 대신 제품 이해와 파트너 문의를 우선한다.

### Perfume Detail Hero

- PDF에서 추출한 실제 제품 이미지를 전체 화면 배경으로 사용하고 `BODY CARE 02 / HAIR PERFUME`, 제품명, 사용 시점과 가격을 첫 화면에 배치한다.
- 가격은 정가 `100,000원`을 취소선으로, 현재 할인가 `90,000원`을 주 가격으로 표시한다.
- 스마트스토어 등록 전에는 구매 링크를 만들지 않고 `스마트스토어 준비 중` 상태와 `구매 문의` CTA를 함께 제공한다.
- `--blush`와 `--blush-mist`는 이 제품 페이지에서만 제한적으로 사용하며 브랜드의 pine, ink, porcelain 구조를 유지한다.

### Product Lineup Band

- 홈페이지의 산야초 제품 챕터 다음에 퍼퓸 제품 챕터를 전체 폭 이미지 밴드로 이어 배치한다.
- 제품 메뉴는 홈페이지 `#products`로 연결해 두 제품군을 순서대로 비교할 수 있게 한다.
- 퍼퓸 밴드는 가격과 `새 제품` 상태, 상세페이지 링크를 보여주되 스마트스토어 구매 링크는 노출하지 않는다.

### Product Price

- 정가, 할인가, 할인율 또는 출시 상태를 하나의 가격 묶음으로 읽히게 한다.
- 정가는 취소선과 보조색, 할인가가 가장 강한 숫자 위계, 스마트스토어 미등록 상태는 텍스트 라벨로 제공한다.
- 가격 표시는 한국 원화 기준이며 배송비, 재고, 판매 기간은 확인 전까지 만들지 않는다.

### Ingredient Ledger

- 성분을 효능 카드로 만들지 않고, 세정 기반 / 데일리 케어 성분 / 산야초 블렌드의 세 개 행으로 설명한다.
- 모든 표현은 전성분 함유 사실과 원료적 특성 범위에 머문다.
- 전체 전성분은 기본으로 닫힌 `details`에서 확인할 수 있게 한다.

### Product Fact Ledger

- 제품명, 용량, 성상, 사용법, 제조업자, 책임판매업자, 제조국을 선형 정의 목록으로 제공한다.
- 근거 자료 사이에 충돌이 있는 정보는 공개 사실로 채택하지 않는다. 현재 용량은 실물 라벨의 `250 ml`를 기준으로 한다.
- 사용상 주의사항은 기본으로 닫힌 `details`에 원문 범위로 제공한다.

### Ritual Steps

- 제품 라벨의 용법·용량을 세 단계로 재구성한다: 충분히 적시기, 3~5 ml로 마사지하기, 깨끗이 헹구기.
- 각 단계 제목은 Text 서체 600, 설명은 `--ink-soft`를 사용해 PC와 모바일에서 또렷하게 읽히게 한다.
- 치료 루틴이나 임의의 대기 시간, 마사지 효과를 추가하지 않는다.

### Product Partnership CTA

- 제품 페이지 마지막에 국내 유통·해외 수출 파트너 문의로 이어지는 짙은 소나무색 전환 영역을 둔다.
- 공개 가격, 인증, 수출 실적, MOQ는 확인 전까지 만들지 않는다.

### Partner Inquiry

- 회사, 이름, 이메일, 관심 시장, 제안 내용을 받는다.
- 모든 필드는 visible label을 가진다.
- 입력 중에는 브라우저 세션에 임시 저장하고, 제출 시 공식 이메일 수신 채널로 전송한다.
- States: empty, focus, invalid, sending, sent, error.

### Product Review Invitation

- 홈페이지 첫 화면과 모바일 메뉴에서 `구매하기`와 `후기 남기기`를 우선 행동으로 노출한다.
- 구매 링크는 특정 제품이 아니라 두 제품을 비교할 수 있는 몸 더 봄 네이버 스토어 메인으로 연결한다.
- 제품 히어로에 후기 영역으로 이동하는 바로가기를 제공하고, 제품 상세정보 다음에는 pine 배경의 고대비 후기 초대 영역을 둔다.
- 후기 초대 영역에는 QR과 밝은 `지금 후기 남기기` 버튼을 함께 제공해 제품 페이지의 주요 행동으로 인지되게 한다.
- QR은 `review.html`을 가리키며, 모바일에서는 같은 링크를 직접 누를 수 있다.
- 후기 페이지는 닉네임, 평점, 사용 경로, 후기 내용을 받고 공용 저장소에 저장해 모든 방문자에게 공개한다.
- 홈페이지 공개 동의와 개인정보·연락처 미포함 확인은 필수로 받는다.
- QR은 pine/porcelain 색상과 넉넉한 quiet zone을 사용해 브랜드 톤과 스캔 안정성을 함께 지킨다.

### Public Review Wall

- 후기 페이지의 첫 콘텐츠로 최신 공개 후기를 보여주고, 작성과 열람을 같은 URL에서 완료한다.
- 목록은 닉네임, 제품, 평점, 작성일, 사용 경로, 후기 본문만 표시하며 이메일·전화번호·주소·링크는 받거나 노출하지 않는다.
- 카드 대신 1px hairline으로 구분한 편집형 목록을 사용하고, 평점은 amber 별과 숫자를 함께 제공한다.
- States: loading, populated, load error. 공개 후기 0건을 알리는 부정적 빈 상태 문구는 노출하지 않고, 기존 실제 후기 목록을 그대로 보여준다.
- 공개 후기는 영구 저장해 최신순으로 누적하며, 첫 12건 이후에는 `후기 더 보기`로 이어 읽을 수 있게 한다.
- 각 후기의 작성자, 제품·경로·날짜, 평점, 본문을 분리해 긴 목록에서도 빠르게 훑을 수 있게 한다. 새 후기 등록 성공 시 목록 맨 앞에 즉시 추가하고 등록 완료 영수증으로 초점을 이동한다.
- 모바일에서는 후기 메타 정보를 두 줄로 재배치하고 작성 CTA를 전체 너비로 제공한다.

### Review Submission Receipt

- 후기 전송 성공 시 입력 폼을 방금 작성한 내용을 담은 편집형 영수증 화면으로 교체한다.
- 제품명, 평점, 후기 본문, 작성자, 작성일을 pine 표면 위에 선형으로 배치하고, 공개 후기 목록에 게시되었음을 명시한다.
- 전환은 `opacity`, `transform`, `filter`만 사용하며 `prefers-reduced-motion`에서는 즉시 교체한다.
- States: hidden, entering, visible. 전송 실패 시에는 폼과 임시 저장 내용을 그대로 유지한다.

### Customer Voice Grid

- 실제 수집 후기 중 향, 거품, 머릿결, 사용 편의처럼 개인이 직접 느낀 사용감만 원문에서 발췌한다.
- 이름, 건강정보, 질환·치료 맥락은 공개하지 않고 연령대와 생활 맥락만 익명으로 표시한다.
- 별점과 구매 인증은 원문에 확인되지 않은 경우 만들지 않는다.
- 후기 아래에 원문 발췌·의미를 유지한 축약 및 정리·개인 사용 경험이라는 범위를 명시한다.

## 7. Motion and Interaction

- Enter: opacity 0 + translateY(24px)에서 700ms로 등장.
- Button: 220ms color transition, pressed scale 0.98.
- Drawer: 420ms `cubic-bezier(0.32, 0.72, 0, 1)` transform, backdrop 240ms opacity.
- Navigation: 260ms background and color transition.
- 모든 공간 이동은 transform과 opacity만 사용한다.
- `prefers-reduced-motion: reduce`에서는 transform reveal과 smooth scroll을 제거한다.
- beui.dev `drawer`에서 Escape, scroll lock, backdrop dismiss, reduced-motion fade를 채택한다.

## 8. Content and Compliance

- 모브랜드는 웰니스 철학을 말하고, 제품은 그 철학의 첫 사례로만 배치한다.
- `탈모 치료`, `발모`, `모근 강화`, `두피질환 개선`, `100% 무자극`, `병원 추천`을 쓰지 않는다.
- `피부 자극지수 0.00`은 시험명, 기관, 기간, 대상자, 개인차와 시험 한계를 함께 표시한다.
- 해외 파트너 영역은 아직 보유하지 않은 국가, 인증, 매출, 유통망을 만들지 않는다.
- 사업자 정보에는 상호, 대표자, 사업자등록번호, 사업장 주소, 전화, 이메일, 통신판매업 신고번호만 공개한다. 생년월일과 계좌번호는 노출하지 않는다.
- 제품 상세 공개 정보는 실물 라벨, 시험보고서, 컴플라이언스 가이드가 서로 일치하는 범위만 사용한다.
- 클린 앤 스테이 퍼퓸은 제공된 PDF의 품목상세정보를 기준으로 제품명, 100 ml, 사용법, 전성분, 제조업자, 책임판매업자, 제조국을 표시한다. 탈모 증상 완화 문구는 품목상세정보의 기능·효과 항목에서만 그대로 제시하고 마케팅 헤드라인이나 치료 표현으로 확대하지 않는다.
- 공개 후기 입력은 20~500자로 제한하고 URL, 이메일, 전화번호, 욕설, 질환·치료·예방·발모 표현을 서버에서 자동 거절한다. 통과한 생활 사용감 후기만 즉시 공개한다.
- 모든 공개 후기 텍스트는 HTML이 아니라 텍스트 노드로 렌더링하며, 저장 API는 허용 목록 기반 필드 검증, honeypot, 최소 작성 시간 검사를 적용한다.
- 익명 등록은 서버가 검증하는 작업 증명과 Blob의 원자적 고정 슬롯을 이용한 IP 해시별 10분 구간당 3회 제한을 함께 적용한다.
- 실물 라벨에는 기능성 문구가 있으나 별도 확인 문서가 정리되기 전까지 공개 카피는 일반 화장품 기준으로 보수 적용한다.
- 상세페이지 생성 이미지에 표기된 `500 ml`는 실물 라벨의 `250 ml`와 불일치하므로 사용하지 않는다.

## 9. Asset Direction

- `hero-morning.webp`: 메인 히어로. 밝은 건축 공간과 제품, 사람의 실루엣.
- `product-water.webp`: 첫 제품 챕터. 젖은 검은 돌과 물, 제품.
- `care-morning.webp`: 배려의 생활 장면. 가족을 위한 작은 선택.
- `product-pair.webp`: 제품 근접 상세.
- `perfume-studio.jpg`: 클린 앤 스테이 퍼퓸 스튜디오 제품 이미지. 제공된 PDF 11페이지에서 원본 JPEG를 추출한다.
- `perfume-detail.jpg`: 클린 앤 스테이 퍼퓸 제품 정면·분사 이미지. 제공된 PDF 1페이지에서 원본 JPEG를 추출한다.
- `perfume-portable.jpg`: 휴대 사용 장면. 제공된 PDF 5페이지에서 원본 JPEG를 추출한다.
- 생성 이미지는 기존 제품 사진을 참고해 만든 콘셉트 자산이며, 실제 촬영본 확보 시 같은 구도로 교체한다.

## 10. Anti-Patterns

- 베이지 일색의 한방 브랜드.
- 녹색 잎과 원료를 과도하게 쌓은 자연주의 상세페이지.
- 동일한 카드 3개를 반복하는 기능 소개.
- 의료 권위나 질환 불안을 이용한 설득.
- 허위 수치, 가짜 파트너 로고, 가짜 수출 실적.
- 보라색 그라디언트, 글로우, 장식 오브, 중첩 카드, 과도한 pill.
