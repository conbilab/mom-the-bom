...
- 현재 제품: 모앤봄 산야초 샴푸. 몸 더 봄 철학을 보여주는 첫 번째 생활 제품으로 다룬다.
...
- `2026-모앤봄-경쟁사분석-전략보고서.md`: 경쟁사의 강한 증거 배치 문법은 배우되, 몸 더 봄은 산야초, 생활 루틴, 조용한 증거, 선물의 언어로 차별화한다.
...
제품 상세페이지에서는 제품 정보 뒤에 후기 작성 QR 진입점을 두고, 별도의 후기 작성 페이지로 연결한다.
...
- 한글 `몸 더 봄`과 영문 `MOM THE BOM`을 세로로 조합한다.
...
- `product-water.webp`를 전체 화면 배경으로 사용하고 제품명, 데일리 리추얼 문장, 검증된 핵심 사실을 첫 화면에 배치한다.
- 핵심 사실은 `250 ml`, `Made in Korea`, `피부 자극지수 0.00`만 사용한다.
- 구매 가격이나 재고 대신 제품 이해와 파트너 문의를 우선한다.

### Ingredient Ledger

- 성분을 효능 카드로 만들지 않고, 세정 기반 / 데일리 케어 성분 / 산야초 블렌드의 세 개 행으로 설명한다.
- 모든 표현은 전성분 함유 사실과 원료적 특성 범위에 머문다.
- 전체 전성분은 기본으로 닫힌 `details`에서 확인할 수 있게 한다.

### Product Fact Ledger

- 제품명, 용량, 성상, 사용법, 제조업자, 책임판매업자, 제조국을 선형 정의 목록으로 제공한다.
- 근거 자료 사이에 충돌이 있는 정보는 공개 사실로 채택하지 않는다. 현재 용량은 실물 라벨의 `250 ml`를 기준으로 한다.
- 사용상 주의사항은 기본으로 닫힌 `details`에 원문 범위로 제공한다.
...
- 제품 페이지 마지막에 국내 유통·해외 수출 파트너 문의로 이어지는 짙은 소나무색 전환 영역을 둔다.
- 공개 가격, 인증, 수출 실적, MOQ는 확인 전까지 만들지 않는다.

### Partner Inquiry

- 회사, 이름, 이메일, 관심 시장, 제안 내용을 받는다.
- 모든 필드는 visible label을 가진다.
- 입력 중에는 브라우저 세션에 임시 저장하고, 제출 시 공식 이메일 수신 채널로 전송한다.
- States: empty, focus, invalid, sending, sent, error.

### Product Review Invitation

- 제품 히어로에 후기 영역으로 이동하는 바로가기를 제공하고, 제품 상세정보 다음에는 pine 배경의 고대비 후기 초대 영역을 둔다.
- 후기 초대 영역에는 QR과 밝은 `지금 후기 남기기` 버튼을 함께 제공해 제품 페이지의 주요 행동으로 인지되게 한다.
- QR은 `review.html`을 가리키며, 모바일에서는 같은 링크를 직접 누를 수 있다.
- 후기 페이지는 닉네임, 평점, 사용 경로, 후기 내용을 받고 공식 이메일 수신 채널로 전송한다.
...
- 사업자 정보에는 상호, 대표자, 사업자등록번호, 사업장 주소, 전화, 이메일, 통신판매업 신고번호만 공개한다. 생년월일과 계좌번호는 노출하지 않는다.
- 제품 상세 공개 정보는 실물 라벨, 시험보고서, 컴플라이언스 가이드가 서로 일치하는 범위만 사용한다.
- 실물 라벨에는 기능성 문구가 있으나 별도 확인 문서가 정리되기 전까지 공개 카피는 일반 화장품 기준으로 보수 적용한다.
- 상세페이지 생성 이미지에 표기된 `500 ml`는 실물 라벨의 `250 ml`와 불일치하므로 사용하지 않는다.
